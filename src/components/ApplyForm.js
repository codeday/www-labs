...
const PROFILE_INFO_SCHEMA = {
  type: "object",
  required: ["givenName", "surname", "email", "minHours", "githubUsername"],
  properties: {
    givenName: {
      type: "string",
    ...
    minHours: {
      type: "string",
      title:
        "What's the minimum number of hours you're able to work on this in a week?",
      oneOf: [
        {
          const: "10",
          title: "10-20",
...
export default function ApplyForm({
  ...
  const applicationsOpen = useDateBetween(
    applicationsStartAt,
    applicationsEndAt,
  );
  const applicationCloseSoon = useDateBetween(
    applicationsEndAt?.minus?.({ days: 3 }),
  );

  if (applicationId || labs?.applicationId)
    return (
      <Box {...props} textAlign="center">
        <Text>
          Your application was received with ID{" "}
          {applicationId || labs.applicationId}.
        </Text>
        <Button
          mr={2}
          onClick={async () => {
  ...
  if (!event || !tags || applicationsOpen === null)
    return (
      <Box {...props}>
        <Spinner />
      </Box>
      ...
      <Box {...props}>
        <Text>
          <center>
            ...
            <Timestamp ts={applicationsStartAt} />
            {" to "}
  ...
  const hasProfile =
    event.studentApplicationSchema &&
    Object.entries(event.studentApplicationSchema).length > 0;
                ...
                try {
                  const result = await apiFetch(
                    ApplyMutation,
                    {
                      ...
                      tags: [...selectedInterests, ...selectedTechnologies].map(
                        (e) => e.id,
                      ),
                      profile: profile,
                      track,
      ...
      >
        <MultiPagePage title="Contact Information">
          <Form
            ...
            uiSchema={{}}
            onChange={(e) => {
              setBasicData(e.formData);
              setBasicErrors(e.errors);
            }}
          ...
          <Heading as="h3" fontSize="md" mt={8}>
            Which timezone do you plan to work from during the program?
          </Heading>
          <TimezoneSelect
        ...
        <MultiPagePage title="Profile">
          <Heading as="h3" fontSize="md" mb={1}>
            Upload your Resume/CV, if you have one:
          </Heading>
          <Box mb={6}>
            <input
              ref={uploadRef}
            ...
            {resume && (
              <Text display="inline-block" color="current.textLight">
                {resume.name}
                <Link ml={2} onClick={() => setResume(null)}>
                  (remove)
                </Link>
              </Text>
            )}
          ...
          {hasProfile && (
            <Form
              ...
              onChange={(e) => {
                setProfile(e.formData);
                setProfileErrors(e.errors);
              }}
              formData={profile}
              disabled={isLoading}
              children={true}
        ...
        <MultiPagePage title="Interests">
          <Heading as="h3" fontSize="md" mb={2}>
            In which of these topics are you interested?
            <Text display="inline" color="red.300">
              *
            </Text>
          </Heading>
          <TagPicker
          ...
          <Heading as="h3" fontSize="md" mb={2} mt={8}>
            Which technologies are you comfortable using?
            <Text display="inline" color="red.300">
              *
            </Text>
          </Heading>
