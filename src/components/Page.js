  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setEventTokens([
        ...
        ...data?.labs?.otherEventTokens.filter(
          (e) => e.event.id !== data?.labs?.event.id,
        ),
      ]);
    })();
  }, [typeof window, query]);
          ...
          <Menu>
            {query?.token && eventTokens.length > 0 ? (
              <Select
                ...
                onChange={(e) => {
                  const route =
                    document.location.pathname.split("/")?.[2] || "s";
              ...
              >
                {eventTokens
                  .sort(
                    (a, b) =>
                      DateTime.fromISO(b.event.startsAt).toSeconds() -
                      DateTime.fromISO(a.event.startsAt).toSeconds(),
                  )
                  .map(({ event, token }) => (
                    <option key={event.id} value={token}>
                      {event.name}
                    </option>
                  ))}
              </Select>
              ...
              <>
                <Button variant="ghost" as="a" href="/schools">
                  Colleges
                </Button>
                <Button variant="ghost" as="a" href="/mentor">
                  Mentors
                </Button>
                <Button variant="ghost" as="a" href="/oss">
                  OSS Maintainers
                </Button>
                <Button
                  variant="solid"
